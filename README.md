...
jobs[옵션][jobID...]
jobs -x command [args]   로 사용할수 있다.
- 옵션
1) -l : 프로세스 그룹 ID를 state 필드 앞에 출력한다.
2) -n : 프로세스 그룹 중에 대표 프로세스 ID를 출력한다.
3) -p : 각 프로세스 ID에 대해 한 행씩 출력한다.
4) command : 지정한 명령어를 실행한다.

-jobs 명령어로 확인할 수 있는 세션의 상태값
1) Running : 작업이 일시 중단되지 않았고 종료하지 않고 계속 진행 중임을 뜻한다.
2) Done : 작업이 완료되어 0을 반환하고 종료했음을 뜻한다.
3) Done (code) : 작업이 정상적으로 완료했으며, 0이 아닌 코드를 반환했음을 뜻한다.
4) Stopped : 작업이 일시 중단됨을 뜻한다.
5) Stopped (SIGTSTP) : SIGTSTP 신호가 작업을 일시 중단했음을 뜻한다.
6) Stopped (SIGSTOP) : SIGSTOP 신호가 일시 중단했음을 뜻한다.
7) Stopped (SIGTTIN) : SIGTTIN 신호가 작업을 일시 중단했음을 뜻한다.
8) Stopped (SIGTTOU) : SIGTTOU 신호가 작업을 일시 중단했음을 뜻한다.

**4) kill**
프로세스를 종료한다.

kill [-s시그널][-a]pid...
kill -l [시그널]        로 사용할 수 있다.
- 옵션
...
## vim 에디터에서 매크로 사용법
1) vim의 중립모드에서 q를 누른 다음 매크로 이름으로 사용할 알파벳을 눌러준다. 그 후 매크로를 기록한 후에 중립모드에서 q를 눌러주면 매크로 저장이 완료된다. 매크로를 재생하려면 중립모드에서 @를 누른 후 재생하고싶은 매크로의 이름을 누른다. 만약 @@를 누르면 제일 마지막에 재생된 매크로가 재실행된다.

2) 매크로 파일로 저장하는방법은 ~/.vimrc 파일을 열어서 let @를 누른 후 매크로 이름을 눌러주고 그 후에 ='까지 친다. 그 후 insert모드에서 Ctrl-R과 Ctrl-R과 매크로 이름을 누르면 매크로가 입력된다. 내용물이 입력 되었으면 마지막으로 '를 눌러주면 된다.
